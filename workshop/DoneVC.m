...
- (void)viewDidLoad {
    ...
    [self.tabBarController setTitle:@"DONE"];
}
...
- (nonnull NSMutableArray *)fetchSavedTasks {
    ...
    if (savedTasks == nil) {
        ...
        for(Task*t in _allTasks){
            if ([t.status isEqualToString:@"Completed"]) {
                [tasks addObject:t];
                }
        }
    }
...
- (void)updateTableViewVisibility {
    if (self.tasksArray.count > 0) {
        ...
        self.doneImg.hidden=YES;
    } else {
        self.table.hidden = YES;
        self.doneImg.hidden=NO;

    }
}
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        // Display alert confirmation before deleting the item
